...
export default function ConsolaPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24, maxWidth: 1100 }}>
      ...
      {(() => {
        // Count truly lost ops: not scheduled AND not done
        const scheduledOps = new Set(segmentos.map((s) => s.op_id));
        const lostOps = operacoes.filter(
          (o) => o.progresso < 100 && !scheduledOps.has(o.op_id),
        );
        if (lostOps.length === 0) return null;
        // Group by molde
        const byMolde = new Map<string, number>();
        for (const o of lostOps) {
          byMolde.set(o.molde, (byMolde.get(o.molde) ?? 0) + 1);
        }
        return (
          <Card style={{ padding: "16px 20px", borderLeft: `4px solid ${T.red}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
              <Num size={28} color={T.red}>{lostOps.length}</Num>
              <div style={{ fontSize: 15, fontWeight: 600, color: T.primary }}>
                operacoes sem planeamento
              </div>
            </div>
            <div style={{ fontSize: 11, color: T.secondary, marginBottom: 10 }}>
              Estas operacoes nao foram agendadas pelo sistema. Verifique se tem maquina compativel.
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {[...byMolde.entries()]
                .sort((a, b) => b[1] - a[1])
                .map(([moldeId, count]) => {
                  const molde = moldes.find((mo) => mo.id === moldeId);
                  const total = molde?.total_ops ?? 0;
                  return (
                    <div key={moldeId} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
                      <Dot color={T.red} size={6} />
                      <span style={{ fontFamily: T.mono, fontWeight: 600, color: T.primary }}>{moldeId}</span>
                      {molde?.cliente && <span style={{ color: T.secondary }}>({molde.cliente})</span>}
                      <span style={{ color: T.tertiary }}>{"\u2014"}</span>
                      <span style={{ color: T.red, fontWeight: 600 }}>{count} sem agendar</span>
                      <span style={{ color: T.tertiary }}>de {total}</span>
                    </div>
                  );
                })}
            </div>
